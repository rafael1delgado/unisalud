<div>
    @foreach($practitioners as $key => $value)
        <div class="card mb-3">
            <div class="card-body">
                ...
                <input type="hidden" name='practitioner_id[]' wire:model='practitioners.{{$key}}.id'>

                <div class="form-row">
                    <fieldset class="form-group col-md-4">
                        <label for="for_organization_id">Organización</label>
                        <select name="organization_id[]" class="form-control"
                                wire:model='practitioners.{{$key}}.organization_id' required
                                >
                            <option value=""></option>
                    ...
                    <fieldset class="form-group col-md-4">
                        ...
                        <select name="profession_id[]" class="form-control"
                                wire:model='practitioners.{{$key}}.profession_id'
                                >
                            <option value=""></option>
                    ...
                    <fieldset class="form-group col-md-3">
                        ...
                        <select name="specialty_id[]" class="form-control"
                                wire:model='practitioners.{{$key}}.specialty_id'
                                >
                            <option value=""></option>
    ...
    <div class="form-row">
        <button type="button" class="btn btn-primary" wire:click.prevent="add({{$i}})"> <i class="fa fa-plus" aria-hidden="true"></i> @if(count($practitioners) == 0) Agregar como funcionario @else Agregar otra organización @endif
        </button>
    </div>
</div>
